...
matching <- matchit(CE ~ elevation+ distCity + distPublic + 
                    ...
                    method = "genetic", # find the nearest neighbors
                    ...
                    caliper=c(elevation=0.7, # The caliper determines how similar the obs. have to be
                              distPublic = 0.7,
                              distCity = 0.7,
                              distTrust = 0.7,
                              distRoad = 0.7))  ## The caliper = "how many std dev" apart (rel. to sd
...
priors.null = get_prior(outcome ~ post + elevation.std + flowAcc.std + slope.std 
                        + spei.std +  (1|year) + (1 |huc12),
                        data = panel, family = 'beta')
...
#ce.full <- brm(outcome ~ CE*post + elevation.std + flowAcc.std + slope.std + spei.std +  (1|year) + (1 |huc12),
ce.naive<- brm(outcome ~ post + elevation.std + flowAcc.std + slope.std + spei.std +  (1|year) + (1 |huc12),
#did.AR <- brm(outcome ~ CE * post + (1 | year) + (system.index | huc12), 
              ...
              prior=priors.null, ## nor priors - 0,1 for any fixed effects; 0.2 for random effects
              #prior=priors.AR, 
              #prior=priors.ce, 
