...
posterior <- did.dev.full%>% 
  ...
  mutate(name = factor(name, levels = c("Targeted condition", "Easement effect", 
                                             "Post implementation", "Intercept", "Dist to GAP 1 or 2", 
                "Slope", "Dist to public", "Dist to city","Dist to road", "Elevation", "Income", "Value",
                "Population change")))
...
value = effect(posterior, "b_value.std")
value
sd(panel$value.std)
