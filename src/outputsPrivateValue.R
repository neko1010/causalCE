...
posterior_beta <- did.ce.full%>% #matched 
#posterior_beta <- did.ce.private%>% 
...
odds_effect80 = function(posts, var.name, int){
  ## filter df for only draws of interest
  draws = posts%>%
    filter(.variable == var.name)
  ## central tendency and SD
...
ce
ce80 = odds_effect80(posterior_beta, "b_CE1", intercept)
ce80
...
val = odds_effect(posterior_beta, "b_value.std", intercept)
flow.acc
#sd(panel$flowAcc)
