...
#                     distPublic.std + distRoad.std + distGAP12.std + income2009.std + pop.delta.std
priors.full = get_prior(dev ~ CE*post + (1|year) + (1 |huc8),
                        data = panel, family = 'binomial')

priors.full$prior[1:10] = "normal (0,1)"
...
did.dev.full <- brm(dev ~ CE*post + (1|year) + (1 |huc8),
              	     data = panel.samp,
              	     #family='beta',
              	     family=bernoulli(),
              		prior=priors.full, 
...
summary(did.dev.full)
plot(did.dev.full)
pp_check(did.dev.full)
r2 = bayes_R2(did.dev.full)
r2

save.image(file = "didDevMatchPart.RData")
